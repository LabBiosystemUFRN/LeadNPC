...
#intersecção dos clusteres
 ...
 # association7<-transc[[1]]@association
 # idx <- !duplicated(t(apply(association7, 1, sort)))
 # teste <- association7[idx,]
 # nrow(teste)
 # nrow(transc[[1]]@association)
# gMaster2 <- graph_from_data_frame(transc[[1]]@association, directed = F)
gMaster <- graph_from_data_frame(assocNoDup, directed = F)
# gMaster3 <- graph_from_data_frame(teste, directed = F)
# gMaster<-att.mapv(g = gMaster, dat = cl12, refcol = 1)
# gMaster2<-att.mapv(g = gMaster2, dat = cl12, refcol = 1)
# gMaster3<-att.mapv(g = gMaster3, dat = cl12, refcol = 1)
V(gMaster)$nodeShape <- "CIRCLE"
E(gMaster)$edgeColor<-"#c9c9c9ff"
# V(gMaster2)$nodeShape <- "CIRCLE"
# E(gMaster2)$edgeColor<-"#c9c9c9ff"
# V(gMaster3)$nodeShape <- "CIRCLE"
# E(gMaster3)$edgeColor<-"#c9c9c9ff"
...
cltRef<-data.frame(c1=c1,c2=c2)
  ...
  for(i in 1:11){
    ...
    degree<-degree(F0, mode = "all", loops = F)
    ...
    for(j in seq(2,nrow(cl)-step,step)){
      ...
      # if(cl[i,2]==0){
      #   if(sum(cl[(i-1):(i+1),2])==0){
      #     clu3<-rbind(clu3,cl[i,])
      #   }
      # }else{
      #   clu3<-rbind(clu3,cl[i,])
      # }
    ...
    # smoothedLine<-stats::smooth.spline(clu3)
    # smoothedLine$y[smoothedLine$y<0]<-0
    # cl <- data.frame(degree = smoothedLine$x, p0 = smoothedLine$y)

    cl<-clu3
    cl$p0[cl$p0==0]<- 1e-15
    cl2<-data.frame(degree=log10(cl$degree),p0=log10(cl$p0))
    # infV<-min(cl2$p0[cl2$p0!=-Inf])-1
    # cl2$p0[cl2$p0==-Inf]<-infV
...
#    g
    ...
    # 
    # g<- ggplot()+
    #   geom_line(data=cl,aes(degree,p0))+
...
# conectiv média ----
  ...
  for(time in 1:2){
    ...
    for(cl in 1:11){
      ...
      # F2<-induced_subgraph(graph=gMaster2, v=V(gMaster2)[V(gMaster2)$name %in% nodesP])
      # F3<-induced_subgraph(graph=gMaster3, v=V(gMaster3)[V(gMaster3)$name %in% nodesP])
      # degree<-degree(gMaster, mode = "out", loops = F)
      # degree2<-degree(gMaster2, mode = "out", loops = F)
      # degree3<-degree(gMaster3, mode = "out", loops = F)
      degree<-degree(F0, mode = "out", loops = F)
      # degree2<-degree(F2, mode = "all", loops = F)
      # degree3<-degree(F3, mode = "all", loops = F)
      # d1<-as.data.frame(degree)
      # d1<-tibble::rownames_to_column(d1)
      # d2<-as.data.frame(degree2)
      # d2<-tibble::rownames_to_column(d2)
      # d3<-as.data.frame(degree3)
      # d3<-tibble::rownames_to_column(d3)
      # 
      # d4<-merge(d1,d2,by="rowname",all = T)
      # d4<-merge(d4,d3,by="rowname",all = T)
  ...
  # for(i in 1:nrow(d4)){
  #     d4$degree4[i]<-nrow(subAssoc[subAssoc$p1==d4$rowname[i],])
  # }
  rm(clTmp)
  conecDir="/home/clovis/Dropbox/Chumbo/figuras/Figura2/conectivity/"
  ref2=3
  # clusters 1 p/ 1----
